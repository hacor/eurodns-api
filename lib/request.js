        ...
        if  (err) {
            ...
            if (json.response && json.response.result) {
                ...
                if (json.response.result._code && ['1000', '1001'].indexOf(json.response.result._code) > -1 ) {
                    ...
                    // The standard, almost every request has a resData block
                    if (json.response.resData) result = json.response.resData

                    // ZondeProfile info has no resData block :( so this needs to be parsed differently
                    if (json.response.name && json.response.name.__prefix === 'zoneprofile'){
                        result = json.response.name
                        result.records = json.response.records
                    }
                    ...
                    //console.log( 'EURODNS-API: Reponse: ' + JSON.stringify(result))
                    cb(null, result)
